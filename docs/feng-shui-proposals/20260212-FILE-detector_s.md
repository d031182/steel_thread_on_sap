# Feng Shui Enhancement Proposal: _detect_detector__detect_stale_references

**Proposal ID**: 20260212-FILE-detector_s  
**Date**: 2026-02-12  
**Status**: PROPOSED  
**Priority**: P2 (MEDIUM)  
...
Detector _detect_stale_references has no corresponding unit test
...
**Why This Agent?**
Enhancement fits FileOrganizationAgent's purpose: 'File structure & organization standards'. Matches 4 scope/example/criteria items (weighted confidence: 1.00).
...
### 1. Detector: `_detect_detector__detect_stale_references`

**What It Detects**:
Issues related to: Detector _detect_stale_references has no corresponding unit test
...
- `tools/fengshui/agents/fileorganization_agent.py`
  - Add _detect_detector__detect_stale_references method
  - Update analyze_module() to call new detector

**Files to Create**:

- `tests/unit/tools/fengshui/test_detect_detector__detect_stale_references.py` - Unit tests for detector

### 3. Testing Plan

**Unit Tests**:
- File: `tests/unit/tools/fengshui/test__detect_detector__detect_stale_references.py`
