...
class AppV2ModuleValidator:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns structured report for Gu Wu test generation:
        - checks: List of 5 validation checks with pass/fail status
        - issues: All detected issues with details
        - summary: Aggregate metrics
        """
        # Map issue types to check names
        check_issue_types = {
            "Scripts Accessible": ["SCRIPTS_NOT_ACCESSIBLE", "SERVER_NOT_RUNNING"],
            "Navigation Consistency": [],  # No specific issue types (informational check)
            "Interface Compliance": ["INCOMPLETE_INTERFACE"],
            "Dynamic Loading Compatibility": ["ES6_EXPORT_INCOMPATIBLE"],
            "SAPUI5 Rendering Safety": []  # Warning-only check
        }
        
        # Build checks list
        checks = []
        for check_name, issue_types in check_issue_types.items():
            check_issues = [i for i in self.issues if i.type in issue_types]
            checks.append({
                "name": check_name,
                "status": "FAIL" if check_issues else "PASS",
                "issues": [i.to_dict() for i in check_issues]
            })
